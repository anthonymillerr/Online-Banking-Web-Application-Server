...
.deny-button:hover {
  ...
  body {
      min-width: 1060px; /* Set the minimum width for the entire body */
    }
  @media screen and (max-width: 1200px) {
      /* Additional styles for screens with a width up to 1060px if necessary */
      body {
        overflow-x: auto; /* Enable horizontal scrollbar if the content overflows */
      }
      .container{
        margin-bottom: 60px;
        margin-top: -70px;
        ...
        <table  class="table table-bordered text-center" style="font-size: 12px;">
            ...
            <?php
                        while ($row = mysqli_fetch_assoc($result)) {
                          if(trim($row['transaction_type']) !== 'Transfer'){
                            echo '<tr>';
